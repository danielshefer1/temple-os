...
e820_entry_t* get_E820_entries(uint64_t count, uint64_t address) {
    ...
    for (size_t i = 0; i < count; ++i) {
        entries[i].base_low = *(uint64_t*)(address + i * sizeof(e820_entry_t));
        entries[i].base_high = *(uint64_t*)(address + i * sizeof(e820_entry_t) + 4);
        entries[i].length_low = *(uint64_t*)(address + i * sizeof(e820_entry_t) + 8);
        entries[i].length_high = *(uint64_t*)(address + i * sizeof(e820_entry_t) + 12);
        entries[i].type = *(uint64_t*)(address + i * sizeof(e820_entry_t) + 16);
    }
    return entries;
}
